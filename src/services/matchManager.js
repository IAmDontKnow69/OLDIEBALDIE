...
async function _renderMatchText(match) {
  ...
  const timeStr = (match.scheduled_at && match.scheduled_at !== 'undefined') ? new Date(match.scheduled_at).toUTCString() : new Date().toUTCString();

  const header = `OLDIEBALDIE MATCH\nMatch at ${timeStr} VS ${match.opponent || 'Unknown'}\nUK TEAM ONLY\n\n`;
...
async function scheduleMatch({ matchInput, notes, requester, channel, guild }) {
  ...
  const matchObj = {
    ...
    notes: notes || '',
    guildId: guildObj ? guildObj.id : (guild ? guild.id : null),
  ...
  try {
    ...
    if (cat && guildObj) {
      ...
      const matchVoice = await guildObj.channels.create({ name: `🔊 Matchday: VS ${matchObj.opponent}`, type: ChannelType.GuildVoice, parent: cat });
      const fanVoice = await guildObj.channels.create({ name: `🏟️ Fan Zone: VS ${matchObj.opponent}`, type: ChannelType.GuildVoice, parent: cat });
